            ...
            if calcType in mr_wfn_method:
                ...
                if calcType in md_wfn_method:
                    ...
                    if (calcType == 'CCSD'):
                        ...
                        ene1 = api.energiesType(unit='u:ElectronVolt')
                        ee_ene1 = api.energyType(type_='gc:totalPotential')
                        ee_ene1.set_valueOf_(float(data.cienergies[-1]))
                        ce_ene1 = api.energyType(type_='gc:correlation')
                        ce_ene1.set_valueOf_(float(data.cienergies[-1])-float(molEE))
                        nn_ene1 = api.energyType(type_='gc:nuclearRepulsion')
                        nn_ene1.set_valueOf_(data.nnenergies[-1])
                        ene1.add_energy(ee_ene1)
                        ene1.add_energy(ce_ene1)
                        ene1.add_energy(nn_ene1)
                        cid1.set_energies(ene1)
                        if hasOrb:
                            cid1.set_waveFunction(wfn1)
                        if hasProp:
                            cid1.set_properties(prop1)
                        if hasNMR:
                            cid1.set_properties(prop2)
                        if hasFreq:
                            cid1.set_vibrationalAnalysis(vib1)
                        mdm1.set_cid(cid1)
                    elif (calcType == 'CISD'):
                        cisd1 = api.resultType(methodology='gc:normal',spinType='gc:'+molSpin, \
                                basisSet='bse:'+basisName)
                        ene1 = api.energiesType(unit='u:ElectronVolt')
                        ee_ene1 = api.energyType(type_='gc:totalPotential')
                        ee_ene1.set_valueOf_(float(data.cienergies[-1]))
                        ...
                        ene1.add_energy(ce_ene1)
                        ene1.add_energy(nn_ene1)
                        cisd1.set_energies(ene1)
                        if hasOrb:
                            cisd1.set_waveFunction(wfn1)
                        if hasProp:
                            cisd1.set_properties(prop1)
                        if hasNMR:
                            cisd1.set_properties(prop2)
                        if hasFreq:
